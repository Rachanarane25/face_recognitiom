    ...
    <div className="max-w-6xl mx-auto">
      <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-lg mb-8">
        <div className="flex justify-between items-start">
            <div>
                <h2 className="text-3xl font-bold text-[#111827]">Live Attendance Session</h2>
                <p className="text-[#374151] mt-1">{course.name} - {unit.name}</p>
                <p className="text-sm text-[#6B7280]">Attendance Radius: {session.radius}m</p>
            </div>
            <button 
                onClick={endSession} 
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 text-center">
            ...
            <p className="text-4xl font-bold text-[#111827] mt-2">{presentStudentIds.size} / {courseStudents.length}</p>
            <p className="text-[#6B7280]">Students Marked Present</p>
        </div>
      </div>
      
      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold mb-4 text-[#111827]">Present Students ({presentStudents.length})</h3>
            {presentStudents.length > 0 ? (
                <ul className="space-y-3 max-h-96 overflow-y-auto">
                    {presentStudents.map(student => (
                        <li key={student.id} className="bg-gray-50 p-3 rounded-md flex items-center space-x-4">
            ...
            ) : <p className="text-[#6B7280]">No students have been marked present yet.</p>}
        </div>
        <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-lg">
            <h3 className="text-xl font-semibold mb-4 text-[#111827]">Absent Students ({absentStudents.length})</h3>
            {absentStudents.length > 0 ? (
                <ul className="space-y-3 max-h-96 overflow-y-auto">
                    {absentStudents.map(student => (
                        <li key={student.id} className="bg-gray-50 p-3 rounded-md flex items-center space-x-4 opacity-60">
                            <img src={student.photo} alt={student.name} className="w-12 h-12 rounded-full object-cover"/>
                            <div>
                                <p className="font-semibold text-[#111827]">{student.name}</p>
                                <p className="text-sm text-[#6B7280]">ID: {student.id}</p>
            ...
            ) : <p className="text-[#6B7280]">All students are present.</p>}
        </div>
      </div>
    </div>
  );
