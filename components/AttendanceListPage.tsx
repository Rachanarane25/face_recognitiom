...
import type { Page, AttendanceRecord, Course, Unit, Venue } from '../types.ts';
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-[#111827]">Attendance Records</h2>
        {attendance.length > 0 && (
          <button 
            onClick={handleExportCSV}
            className="bg-[#10B981] hover:bg-[#059669] text-white font-bold py-2 px-4 rounded transition-colors flex items-center space-x-2"
      ...
      {attendance.length > 0 && (
         <div className="mb-6">
          ...
          <input
            ...
            className="w-full bg-gray-50 border border-gray-300 rounded-md shadow-sm py-2 px-3 text-[#111827] focus:outline-none focus:ring-2 focus:ring-[#10B981] focus:border-transparent"
      ...
      {sortedDates.length > 0 ? (
        <div className="space-y-8">
          {sortedDates.map(date => (
            <div key={date} className="bg-white border border-gray-200 p-6 rounded-lg shadow-lg">
              <h3 className="text-xl font-semibold mb-4 border-b border-gray-200 pb-2 text-[#111827]">{date}</h3>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {groupedAttendance[date].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map(record => (
                      <tr key={record.studentId + record.timestamp} className="bg-white border-b border-gray-200 hover:bg-gray-50">
                        <td className="px-6 py-4 font-medium text-[#111827] whitespace-nowrap">{record.studentName}</td>
                        ...
                        <td className="px-6 py-4">
                            ...
                            <div className="text-xs text-[#6B7280]">{getDataNameById(units, record.unitId)}</div>
                        </td>
                        <td className="px-6 py-4">
                            {record.sessionLocation ? (
                                <div className="text-xs">
                                    ...
                                    {record.sessionRadius && <div className="text-[#6B7280]">Radius: {record.sessionRadius}m</div>}
                                </div>
                            ) : (
        ...
        <div className="text-center bg-white border border-gray-200 p-10 rounded-lg">
          ...
          {attendance.length === 0 && (
            <button onClick={() => setPage('LECTURER_DASHBOARD')} className="mt-4 bg-[#10B981] hover:bg-[#059669] text-white font-bold py-2 px-4 rounded transition-colors">
      ...
      <button onClick={() => setPage('LECTURER_DASHBOARD')} className="mt-8 bg-[#6B7280] hover:bg-gray-600 text-white font-bold py-2 px-4 rounded transition-colors">
        Back to Dashboard
      </button>
    </div>
  );
