...
import type { User, Student, AttendanceRecord, StudentSession, Course, Unit, Venue } from '../types.ts';
import { useCamera } from '../hooks/useCamera.ts';
import { compareFaces } from '../utils/gemini.ts';
import { calculateDistance } from '../utils/geolocation.ts';
import { getGeoLocation } from '../utils/location.ts';
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-gray-100 p-6 rounded-lg shadow-lg">
                <h4 className="font-bold text-lg mb-4 text-[#374151]">Monthly Attendance</h4>
...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, students, attendance, setAttendance, studentSession, courses, units, venues }) => {
    ...
    if (isAlreadyMarked || status === 'CONFIRMED') {
        content = (
            <div className="text-center">
                ...
                <p className="text-gray-300 mt-2">You have been successfully marked present for this session.</p>
            </div>
        );
    } else {
        switch(status) {
            case 'READY':
                content = (
                     <div className="text-center">
                        ...
                        <p className="text-gray-300 mt-2">The system will first check your location and then start the camera.</p>
                        <button onClick={handleStartAttendanceProcess} className="mt-6 w-full bg-[#10B981] hover:bg-[#059669] text-white font-bold py-3 px-4 rounded-lg transition-colors text-lg">
                            Mark Attendance
                        </button>
                    </div>
                ); break;
             case 'CHECKING_LOCATION':
                content = (
                    <div className="text-center">
                        ...
                        <p className="text-gray-300 mt-2">Please wait while we verify you are in the attendance zone.</p>
                    </div>
                ); break;
            case 'CAPTURING':
                content = (
                    <div>
                        <div className="aspect-video bg-gray-900 rounded-md overflow-hidden relative">
                         ...
                         <button 
                            ...
                            className="mt-4 w-full bg-[#10B981] hover:bg-[#059669] text-white font-bold py-3 px-4 rounded-lg transition-colors text-lg disabled:bg-gray-600 disabled:cursor-not-allowed"
                            aria-live="polite"
                         >
            ...
            case 'VERIFYING':
                content = (
                    <div className="text-center">
                        ...
                        <p className="text-gray-300 mt-2">Please wait while we compare your photo.</p>
                    </div>
                ); break;
            case 'SUBMITTING':
                 content = (
                    <div className="text-center">
                        ...
                        <p className="text-gray-300 mt-2">Submitting your geotagged attendance.</p>
                    </div>
                ); break;
             case 'OFFLINE_QUEUED':
                content = (
                    <div className="text-center">
                        ...
                        <p className="text-gray-300 mt-2">You are offline. Your attendance will be submitted automatically when you reconnect.</p>
                    </div>
                ); break;
            case 'ERROR':
                content = (
                    <div className="text-center">
                        ...
                        <div className="mt-6 flex flex-col sm:flex-row gap-3">
                            <button onClick={handleRetry} className="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded-lg">
                                Try Again
                            </button>
                            {isDistanceError && (
    ...
    return (
      <div className="bg-gradient-to-r from-[#10B981] to-emerald-600 rounded-lg shadow-lg p-6 mb-8">
        <div className="text-center mb-4">
            ...
            <p className="text-emerald-100">Session for <strong>{studentSession?.unit.name}</strong> is active.</p>
        </div>
  ...
  if (!studentData) {
    return <div className="text-center p-8 bg-white border border-gray-200 rounded-lg"><p className="text-xl text-yellow-500">Could not find student data.</p></div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className={`bg-white border border-gray-200 rounded-lg shadow-lg p-8 ${isSessionActiveForStudent ? 'mt-8' : ''}`}>
        <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left">
          <img src={studentData.photo} alt={studentData.name} className="w-32 h-32 rounded-full object-cover border-4 border-gray-200 mb-4 md:mb-0 md:mr-8"/>
          <div>
            <h2 className="text-3xl font-bold text-[#111827]">{studentData.name}</h2>
            <p className="text-[#6B7280]">ID: {studentData.id}</p>
            <p className="text-[#6B7280]">Course: {getDataNameById(courses, studentData.courseId)}</p>
        ...
        <div className="mt-8 pt-8 border-t border-gray-200">
            <h3 className="text-2xl font-bold text-[#111827] mb-6">My Attendance Analytics</h3>
            <AttendanceVisuals 
...
// --- SVG Icons ---
const Spinner = () => (
    <svg className="animate-spin h-12 w-12 text-[#10B981] mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
