...
const AttendanceVisuals: React.FC<{ 
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className={`p-6 rounded-lg shadow-lg flex flex-col justify-center items-center text-center border-b-4 ${statusColor} bg-gray-100`}>
                <div className="text-5xl font-bold text-[#111827]">{percentage.toFixed(0)}%</div>
                <div className="text-lg font-semibold text-[#374151] mt-2">Overall Attendance</div>
            </div>

            <div className="lg:col-span-2 bg-gray-100 p-6 rounded-lg shadow-lg">
                <h4 className="font-bold text-lg mb-4 text-[#374151]">Monthly Presence</h4>
                    ...
                    <div className="flex justify-around items-end h-40 space-x-2 pt-4">
                        {monthlyData.labels.slice(-6).map((label, index) => (
                            <div key={label} className="flex flex-col items-center flex-1 h-full group">
                                <div className="text-[#111827] font-bold text-sm mb-1 opacity-0 group-hover:opacity-100 transition-opacity">{monthlyData.data.slice(-6)[index]}</div>
                                <div className="w-full bg-[#10B981] rounded-t-md hover:bg-[#059669] transition-all duration-300 ease-in-out" style={{ height: `${(monthlyData.data.slice(-6)[index] / Math.max(...monthlyData.data, 1)) * 100}%` }}></div>
                                <div className="text-xs text-[#6B7280] mt-2">{label}</div>
                            </div>
                        ))}
                    </div>
                ) : <p className="text-[#6B7280] h-40 flex items-center justify-center">Not enough data for chart.</p>}
            </div>

            <div className="lg:col-span-3 bg-gray-100 p-6 rounded-lg shadow-lg">
                <h4 className="font-bold text-lg mb-4 text-[#374151]">Attendance Heatmap (Last 4 Months)</h4>
                    ...
                    {calendarMonths.map(monthDate => {
                        ...
                        return (
                            <div key={monthDate.toISOString()} className="text-center">
                                <h5 className="font-semibold mb-2 text-[#374151]">{monthDate.toLocaleString('default', { month: 'long' })}</h5>
                                <div className="grid grid-cols-7 gap-1.5 text-xs">
                                    {['S','M','T','W','T','F','S'].map(d => <div key={d} className="font-bold text-[#6B7280]">{d}</div>)}
                                    ...
                                    {Array.from({ length: daysInMonth }).map((_, day) => {
                                        ...
                                        return (
                                            <div key={day} className={`w-full aspect-square rounded-sm transition-colors flex items-center justify-center ${isPresent ? 'bg-emerald-500/30' : 'bg-gray-200'} ${isToday ? 'ring-2 ring-[#10B981]' : ''}`} title={date.toDateString()}>
                                                {isPresent && <TickIcon />}
                                            </div>
                                        )
                                    })}
...
const LecturerDashboard: React.FC<LecturerDashboardProps> = ({ 
  ...
  const renderLecturerAttendanceUI = () => {
    ...
    if (isAlreadyMarked || status === 'CONFIRMED') {
        content = (
            <div className="text-center">
                ...
                <p className="text-gray-300 mt-2">You have been successfully marked present.</p>
            </div>
        );
    } else {
        switch(status) {
            case 'READY': content = (
                <div className="text-center">
                    ...
                    <p className="text-gray-300 mt-2">Click below to start your camera.</p>
                    <button onClick={handleStartCamera} className="mt-6 w-full bg-[#10B981] hover:bg-[#059669] text-white font-bold py-3 px-4 rounded-lg text-lg">Start Camera</button>
                </div>
            ); break;
            case 'CAPTURING': content = (
                <div>
                    ...
                    <button onClick={handleSubmit} disabled={!isVideoReady} className="mt-4 w-full bg-[#10B981] hover:bg-[#059669] text-white font-bold py-3 px-4 rounded-lg text-lg disabled:bg-gray-600">{isVideoReady ? 'Verify & Submit' : 'Camera starting...'}</button>
                </div>
            ); break;
            ...
            case 'ERROR': content = (
                <div className="text-center">
                    ...
                    <div className="mt-6 flex flex-col sm:flex-row gap-3">
                        <button onClick={handleRetry} className="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded-lg">Try Again</button>
                        {isDistanceError && (
                            <button onClick={() => setShowErrorMap(!showErrorMap)} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg">
    ...
    return (
      <div className="bg-gradient-to-r from-[#10B981] to-emerald-600 rounded-lg shadow-lg p-6 mb-8">
        <div className="text-center mb-4">
            <h2 className="text-2xl font-bold text-white">Faculty Attendance Session</h2>
            <p className="text-emerald-100">Session for <strong>{facultySession?.name}</strong> is active.</p>
        </div>
        <div className="bg-emerald-800/50 rounded-lg p-4 min-h-[250px] flex items-center justify-center">
            {content}
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-6xl mx-auto">
        ...
        <h2 className="text-3xl font-bold mb-6 text-[#111827]">Lecturer Dashboard</h2>
        <div className="grid md:grid-cols-2 gap-8 mb-8">
            <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-lg">
                <h3 className="text-xl font-semibold mb-4 text-[#111827]">Start New Student Session</h3>
                <div className="space-y-4">
                    {/* Course & Unit Selection */}
                    <div>
                        <label htmlFor="course-select" className="block text-sm font-medium text-[#374151]">1. Select Course & Unit</label>
                        <select id="course-select" value={selectedCourseId} onChange={e => {setSelectedCourseId(e.target.value); setSelectedUnitId('');}} className="mt-1 block w-full bg-gray-50 border border-gray-300 rounded-md shadow-sm py-2 px-3 text-[#111827] focus:outline-none focus:ring-2 focus:ring-[#10B981] focus:border-transparent">
                            <option value="">Select a Course</option>
                            {courses.map(course => <option key={course.id} value={course.id}>{course.name}</option>)}
                        </select>
                        <select id="unit-select" value={selectedUnitId} onChange={e => setSelectedUnitId(e.target.value)} disabled={!selectedCourseId} className="mt-2 block w-full bg-gray-50 border border-gray-300 rounded-md shadow-sm py-2 px-3 text-[#111827] focus:outline-none focus:ring-2 focus:ring-[#10B981] focus:border-transparent disabled:bg-gray-200">
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-[#374151]">2. Set Location & Radius</label>
                        <button onClick={() => { setIsLocating(true); setLocationError(''); getGeoLocation().then(pos => { setLocation(pos); setLocationError('')}).catch(err => setLocationError(err.message)).finally(() => setIsLocating(false))}} disabled={isLocating} className="mb-2 w-full bg-[#6B7280] hover:bg-gray-600 text-white font-bold py-2 px-4 rounded transition-colors disabled:bg-gray-400">
                        ...
                        <p className="text-xs text-[#6B7280] mb-1">Drag the marker or click the map to fine-tune the location.</p>
                        <div id="lecturer-map-container" className="h-64 w-full bg-gray-200 rounded-lg" aria-label="Map showing current location for attendance geofencing">
                    ...
                    {error && <p className="text-red-600 text-sm">{error}</p>}
                     <button onClick={handleStart} disabled={isLocating || !!locationError} className="w-full bg-[#10B981] hover:bg-[#059669] text-white font-bold py-2 px-4 rounded transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed">
                        {isLocating ? 'Getting Location...' : 'Start Student Session'}
                    </button>
                </div>
            </div>
            <div className="bg-white border border-gray-200 p-6 rounded-lg shadow-lg flex flex-col items-center justify-center">
                 <h3 className="text-xl font-semibold mb-4 text-[#111827]">Manage Student Attendance</h3>
                 <p className="text-[#6B7280] mb-6 text-center">View, manage, and export attendance records for all student sessions.</p>
                 <button onClick={() => setPage('VIEW_ATTENDANCE')} className="w-full bg-[#6B7280] hover:bg-gray-600 text-white font-bold py-2 px-4 rounded transition-colors">
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-6">
            <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left mb-8">
                <img src={user.photo} alt={user.name} className="w-24 h-24 rounded-full object-cover border-4 border-gray-200 mb-4 md:mb-0 md:mr-6"/>
                <div>
                    <h2 className="text-2xl font-bold text-[#111827]">My Attendance Analytics</h2>
                    <p className="text-[#6B7280]">A summary of your attendance for official sessions.</p>
            ...
            <div className="mt-8 pt-8 border-t border-gray-200">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-2xl font-bold text-[#111827]">My Attendance History</h3>
                     {currentUserAttendance.length > 0 && (
                        <button onClick={handleExportCSV} className="bg-[#10B981] hover:bg-[#059669] text-white font-bold py-2 px-4 rounded transition-colors flex items-center space-x-2">
                ...
                {currentUserAttendance.length > 0 ? (
                    ...
                    <table className="w-full text-left">
                        <thead className="text-xs text-[#6B7280] uppercase bg-gray-50 sticky top-0">
                        <tr>
                        ...
                        {currentUserAttendance.map(record => (
                            <tr key={record.timestamp} className="bg-white border-b border-gray-200 hover:bg-gray-50">
                            <td className="px-6 py-4 font-medium text-[#111827]">{new Date(record.timestamp).toLocaleDateString()}</td>
                    ...
                    <p className="text-[#6B7280] text-center">You have no attendance records yet.</p>
                )}
            </div>
        </div>
    </div>
...
const ErrorIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-red-500 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>);
