...
const INITIAL_VENUES: Venue[] = [
  ...
  { id: 'cl-a', name: 'Computer Lab A', latitude: 34.0525, longitude: -118.2440 },
  { id: 'cr-5', name: 'Conference Room 5', latitude: 34.0520, longitude: -118.2435 },
];
...
const App: React.FC = () => {
  ...
  const [units, setUnits] = useLocalStorage<Unit[]>('units', INITIAL_UNITS);
  const [venues, setVenues] = useLocalStorage<Venue[]>('venues', INITIAL_VENUES);
  const [attendance, setAttendance] = useLocalStorage<AttendanceRecord[]>('attendance', []);
  
  const [studentSession, setStudentSession] = useLocalStorage<StudentSession | null>('studentSession', null);
  const [facultySession, setFacultySession] = useLocalStorage<FacultySession | null>('facultySession', null);
  const [lecturerAttendance, setLecturerAttendance] = useLocalStorage<LecturerAttendanceRecord[]>('lecturerAttendance', []);
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'TAKE_ATTENDANCE':
        if (!studentSession) {
          setCurrentPage('LECTURER_DASHBOARD');
          return <LecturerDashboard user={currentUser} setPage={setCurrentPage} onStartStudentSession={handleStartStudentSession} courses={courses} units={units} venues={venues} facultySession={facultySession} lecturerAttendance={lecturerAttendance} setLecturerAttendance={setLecturerAttendance} />;
        }
      ...
      case 'TAKE_LECTURER_ATTENDANCE':
          if (!facultySession) {
            setCurrentPage('ADMIN_DASHBOARD');
            return <AdminDashboard setPage={setCurrentPage} users={users} setUsers={setUsers} students={students} courses={courses} setCourses={setCourses} units={units} setUnits={setUnits} venues={venues} setVenues={setVenues} attendance={attendance} onStartFacultySession={handleStartFacultySession} />;
          }
          return <TakeLecturerAttendancePage
      ...
      case 'VIEW_LECTURER_ATTENDANCE':
          return <ViewLecturerAttendancePage 
            setPage={setCurrentPage}
            attendance={lecturerAttendance}
          />
      ...
      default:
        // Fallback to role-appropriate dashboard
        switch(currentUser.role) {
            case 'ADMIN': return <AdminDashboard setPage={setCurrentPage} users={users} setUsers={setUsers} students={students} courses={courses} setCourses={setCourses} units={units} setUnits={setUnits} venues={venues} setVenues={setVenues} attendance={attendance} onStartFacultySession={handleStartFacultySession} />;
            case 'LECTURER': return <LecturerDashboard user={currentUser} setPage={setCurrentPage} onStartStudentSession={handleStartStudentSession} courses={courses} units={units} venues={venues} facultySession={facultySession} lecturerAttendance={lecturerAttendance} setLecturerAttendance={setLecturerAttendance} />;
            case 'STUDENT': return <StudentDashboard user={currentUser} students={students} attendance={attendance} setAttendance={setAttendance} studentSession={studentSession} courses={courses} units={units} venues={venues}/>;
            default: return <LoginPage users={users} setCurrentUser={setCurrentUser} />;
        }
    }
  };

  return (
    <div className="min-h-screen bg-[#FAFAF9] text-[#374151] font-sans">
